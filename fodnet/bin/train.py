...
"""
from fodnet.options.train_options import TrainOptions
from fodnet.data import create_dataset
from fodnet.models import create_model
from tqdm import tqdm
from fodnet.util.visualizer import Visualizer
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.epoch_end + 1):

        t = tqdm(dataset, desc='Epoch %d' % epoch)
        model.epoch = epoch
        model.total_metric = 0.0
        ...
        for i, data in enumerate(t):  # inner loop within one epoch
            ...
            with torch.no_grad():
                ...
                t.set_description(
                    "Total loss: {0:.6f} Step loss: {1:.6f}".format(
                        model.total_metric / (i + 1), model.loss_total.item()
                    )
                )
                if total_iters % opt.print_freq == 0:
                    visualizer.plot_current_losses_tensorboard(
                        model.get_current_losses(), global_step=(epoch - 1) * dataset_size + i
                    )
