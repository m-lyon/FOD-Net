...
def create_fods(input_path, output_path, bvecs, bvals, brain_mask, csd_type, normalisation=True):
    ...
    WM_FODs_normalised_path = os.path.join(output_path, 'WM_FODs_normalised.nii.gz')
    GM_FODs_normalised_path = os.path.join(output_path, 'GM_FODs_normalised.nii.gz')
    CSF_FODs_normalised_path = os.path.join(output_path, 'CSF_FODs_normalised.nii.gz')
...
def main():
    ...
    for i, subject_id in enumerate(subject_ids):
        ...
        LARDI_dwi_path = os.path.join(
            hcp_dataset_path, subject_id, "LARDI_data", 'data_b1000_g32.nii.gz'
        )
        LARDI_bvecs_path = os.path.join(
            hcp_dataset_path, subject_id, "LARDI_data", 'data_b1000_g32_bvecs'
        )
        LARDI_bvals_path = os.path.join(
            hcp_dataset_path, subject_id, "LARDI_data", 'data_b1000_g32_bvals'
        )
